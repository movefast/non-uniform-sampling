...
# TODO: need to consolidate the definition of env
env = 'Blocking_Maze'
...
env_infos = {
    'Blocking_Maze': {
        ...
        # difficult
#         "obstacles": to_list([*zip([2]*7, range(1,8))]),
        "doors": {tuple():[]},
    },
#     'Shortcut_Maze': {
#         "maze_dim": [6, 8],
...
num_episodes = 300
from maze.run_single_job import num_runs
from maze.write_maze_jobs import params_to_search
    ...
    for metric_name in ["all_reward_sums"]:
        ...
        for env in env_infos:
            for i, agent_name in enumerate(filtered_agent_list):
                ...
                for j, (_, algorithm) in enumerate(sorted_agent_name_pairs[:10]):
        ...
        plt.savefig(ROOT_DIR/f'maze/plots/top_5_{metric_name}.png')

        # plot best learning curves
        fig, ax = plt.subplots(figsize=(20,10))
        for algorithm in best_agent_names:
            plot_metric(metrics, ax, env, algorithm, metric_name)
        plt.plot([0,299], [-50, -50])
        ...
        plt.savefig(ROOT_DIR/f'maze/plots/{metric_name}.png')
...
def plot_parameter_sensitivity(metrics):
    ...
    plt.savefig(ROOT_DIR/f'maze/plots/param_study_{metric_name}.png')
...
def plot_step_size_sensitivity(metrics):
    ...
    for metric_name in ["all_reward_sums"]:
        ...
        for agent_type in agent_list_for_param:
            ...
            #     ax.set_xscale("log")
    plt.suptitle(f"Sensitivity Analysis in 100-state RandomWalk ({metric_name})")
    plt.savefig(ROOT_DIR/f'maze/plots/param_study_{metric_name}_step_size.png')
