    ...
    def batch_train(self):
        ...
        for _ in range(self.num_meta_update):
            ...
            if self.use_grad_norm:
                ...
                # 2) calculate grad_norm manually
                ...
                with torch.no_grad():
                    ...
                    # 2) for each !!! this update does not update geo_weights
                    # for i in range(self.batch_size):
                    #     self.buffer.update(idxs[i], np.power(errors[i].item(), self.per_power))
                    # need to change this to make it work with single update
                    # self.buffer.geo_weights += errors.mean().item() ** self.geo_alpha
                self.optimizer.zero_grad()
                loss.backward()
                for param in self.nn.parameters():
                    param.grad.data.clamp_(-1, 1)
