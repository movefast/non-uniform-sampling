...
class LinearAgent(agent.BaseAgent):
    def agent_init(self, agent_init_info):
        ...
        self.geo_buffer = ReplayMemory(self.buffer_size, self.buffer_alpha, self.buffer_beta, self.beta_increment, self.p)
        self.tau = 0.5
        self.updates = 0
    ...
    def agent_step(self, reward, state):
        ...
        self.geo_buffer.add(self.prev_state, self.prev_action, state, action, reward, self.discount)
    ...
    def agent_end(self, reward, state, append_buffer=True):
        ...
        if append_buffer:
            ...
            self.geo_buffer.add(self.prev_state, self.prev_action, state, 0, reward, 0)
        if len(self.buffer) > self.batch_size:
            self.batch_train()
    ...
    def batch_train(self):
        ...
        transitions, idxs, is_weight = self.geo_buffer.sample_geo(self.batch_size)
        batch = Transition(*zip(*transitions))
        state_batch = torch.cat(batch.state)
        action_batch = torch.LongTensor(batch.action).view(-1, 1).to(device)
        new_state_batch = torch.cat(batch.new_state)
