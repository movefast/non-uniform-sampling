...
class ReplayMemory(object):
    ...
    def __init__(self, capacity, alpha=None, beta=None, beta_increment=None, p=None, cer=False):
        self.capacity = capacity
        self.memory = sliceable_deque([])
        self.position = 0
        self.p = p or 0.5
        if cer:
            temp = [(self.p)*(1-self.p)**n for n in range(self.capacity-1)]
            temp.insert(0, 1.)
            self.geo_weights = np.flip(temp).copy()
            self.weights_sum = self.geo_weights[:-1].sum()
        else:
            temp = [(self.p)*(1-self.p)**n for n in range(self.capacity)]
            self.geo_weights = np.flip(temp).copy()
