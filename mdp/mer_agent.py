...
class LinearAgent(agent.BaseAgent):
    ...
    #     for idx in range(len(state_batch)):
    #         current_q = self.nn(state_batch[idx:idx+1])
    #         q_learning_action_values = current_q.gather(1, action_batch[idx:idx+1])
    #         with torch.no_grad():
    #             # ***
    #             # new_q = self.target_nn(new_state_batch)
    #             new_q = self.nn(new_state_batch[idx:idx+1])
    #         # max_q = new_q.max(1)[0]
    #         # max_q = new_q.mean(1)[0]
    #         max_q = new_q.gather(1, new_action_batch[idx:idx+1]).squeeze_()
    #         target = reward_batch[idx:idx+1]
    #         target += discount_batch[idx:idx+1] * max_q
